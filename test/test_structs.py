...
from lyncs_cppyy import nullptr, array_to_pointers
...
def test_assign_zero(lib):
    for struct in dir(structs):
        print(struct, type(struct),struct == "Struct", struct.startswith("_"),isinstance(struct, Enum),struct == "Enum")
        ...
        print("TET KYS", params.keys())
        for key in params.keys():
            typ = getattr(structs, struct)._types[key]
            obj = getattr(structs, typ) if typ in dir(structs) else int
            val = 0
...
def test_assign_something(lib):
    ...
    print("pre inv set", mp.n_level, getattr(lib.newQudaMultigridParam(),"n_level"), lib.QUDA_MAX_MG_LEVEL)
    mp.invert_param = cppyy.ll.addressof(ip.quda)
    ip.split_grid = list(range(lib.QUDA_MAX_DIM))
    print("post inv set",mp.n_level, hasattr(ip.split_grid,"shape"))
    mp.printf()
    ...
    print(ip.b_5, [ip.b_5[i] for i in range(lib.QUDA_MAX_DWF_LS)])
    # array of basic types probably works
    ip.split_grid = list(range(lib.QUDA_MAX_DIM))
    # hmm, so 'char []' has not shape info => not recognized as an array but as a str via cppyy?
    ...
    mp.printf()
    # setitems does not work as it works wiht subviews, I suppose
    ...
    mp.printf()
    l2 = [[i+j+5 for j in range(lib.QUDA_MAX_DIM)] for i in range(lib.QUDA_MAX_MG_LEVEL)]
    lib.set2Darray[int,lib.QUDA_MAX_DIM,lib.QUDA_MAX_MG_LEVEL](mp.geo_block_size,np.asarray(l2,dtype="i"))#has no effect...
    
    for i in range(lib.QUDA_MAX_MG_LEVEL):
        for j in range(lib.QUDA_MAX_DIM):
            print("geo",i,j,":",mp.geo_block_size[i][j], "vs", l[i][j] +5)

    lib.print2Darray[int,lib.QUDA_MAX_DIM,lib.QUDA_MAX_MG_LEVEL](mp.geo_block_size)
    mp.printf()
