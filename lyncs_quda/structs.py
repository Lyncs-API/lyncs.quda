...
class QudaGaugeParam(Struct):
    ...
    - struct_size : Size of this struct in bytes.  Used to ensure that the host application and QUDA see the same struct size
    ...
    - overwrite_gauge : When computing gauge, should we overwrite it or accumulate to it
    - overwrite_mom : When computing momentum, should we overwrite it or accumulate to it
...
class QudaInvertParam(Struct):
    ...
    - tm_rho : Hasenbusch mass shift applied like twisted mass to diagonal (but not inverse)
    ...
    - clover_csw : Csw coefficient of the clover term
...
class QudaMultigridParam(Struct):
    ...
    - block_ortho_two_pass : Whether to do passes at block orthogonalize in fixed point for improved accuracy
    ...
    - allow_truncation : Whether or not to let MG coarsening drop improvements, for ex dropping long links in small aggregation dimensions
    - staggered_kd_dagger_approximation : Whether or not to use the dagger approximation for the KD preconditioned operator
...
class QudaGaugeObservableParam(Struct):
    ...
    _types = {'struct_size': 'size_t', 'su_project': 'QudaBoolean', 'compute_plaquette': 'QudaBoolean', 'plaquette': 'double [ 3 ]', 'compute_qcharge': 'QudaBoolean', 'qcharge': 'double', 'energy': 'double [ 3 ]', 'compute_qcharge_density': 'QudaBoolean', 'qcharge_density': 'void *'}
...
class QudaBLASParam(Struct):
    ...
    _types = {'struct_size': 'size_t', 'trans_a': 'QudaBLASOperation', 'trans_b': 'QudaBLASOperation', 'm': 'int', 'n': 'int', 'k': 'int', 'lda': 'int', 'ldb': 'int', 'ldc': 'int', 'a_offset': 'int', 'b_offset': 'int', 'c_offset': 'int', 'a_stride': 'int', 'b_stride': 'int', 'c_stride': 'int', 'alpha': 'double_complex', 'beta': 'double_complex', 'batch_count': 'int', 'data_type': 'QudaBLASDataType', 'data_order': 'QudaBLASDataOrder'}
